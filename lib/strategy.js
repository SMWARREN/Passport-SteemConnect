...
class SteemConnectStrategy {
  ...
  getOAuthAccessToken(code, params, callback) {
    ...
    const url = scope.includes('offline') ? `${this._steemconnect._accessTokenUrl}?refresh_token=${code}&scope=${scope}` : `${this._steemconnect._accessTokenUrl}?access_token=${code}&scope=${scope}`;
    return new Promise((resolve, reject) => {
      fetch(url, {
        ...
        method: 'POST',
        body: JSON.stringify({ client_secret: this._steemconnect._clientSecret }),
      })
      ...
      if(!res.error_description){
        ...
        callback(false, null, null, null, params);
      }
    }).catch((err) => {
