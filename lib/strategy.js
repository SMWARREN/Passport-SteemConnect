...
class SteemConnectStrategy {
  ...
  getOAuthAccessToken(code, params, callback) {
    ...
    let url = scope.includes('offline') ? `${this.tokenURL}?refresh_token=${code}&scope=${scope}` : `${this.tokenURL}?access_token=${code}&scope=${scope}`;
    return new Promise((resolve, reject) => {
      fetch(url, {
        ...
        method: 'POST',
        body: JSON.stringify({ client_secret: this.clientSecret }),
      })
        .then(r => {
          if (r.status !== 200) {
